...
describe("Test format_progress_message", function()
    it("plain string returns as-is", function()
        local result = argv_formatter.format_progress_message("Running task...")
        assert.are.same("Running task...", result)
    end)

    it("non-json string returns as-is", function()
        local result = argv_formatter.format_progress_message("not a json string")
        assert.are.same("not a json string", result)
    end)

    it("json without run_process keys returns as-is", function()
        local result = argv_formatter.format_progress_message('{"status": "ok", "progress": 50}')
        assert.are.same('{"status": "ok", "progress": 50}', result)
    end)

    it("json with command_line key formats as command", function()
        local msg = vim.json.encode({ command_line = "ls -la | grep foo" })
        local result = argv_formatter.format_progress_message(msg)
        assert.are.same("ls -la | grep foo", result)
    end)

    it("json with argv key formats with quoting", function()
        local msg = vim.json.encode({ argv = { "cat", "my file.txt" } })
        local result = argv_formatter.format_progress_message(msg)
        assert.are.same('cat "my file.txt"', result)
    end)

    it("json with command key formats as command", function()
        local msg = vim.json.encode({ command = "ls" })
        local result = argv_formatter.format_progress_message(msg)
        assert.are.same("ls", result)
    end)

    it("invalid json returns original message", function()
        local result = argv_formatter.format_progress_message('{"command_line": broken json')
        assert.are.same('{"command_line": broken json', result)
    end)
end)
