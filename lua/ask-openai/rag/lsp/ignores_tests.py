...
def test_package_lock_is_ignored(tmp_root):
    ...
    assert _is_gitignored(tmp_root / "package-lock.json", tmp_root, RagConfig.default())
    assert _is_gitignored(tmp_root / "uv.lock", tmp_root, RagConfig.default())

def test_all_paths_ignored_for_asterisk_dot_extension_pattern(tmp_root):
    # absolute path that IS relative to root_path
    assert _is_gitignored(tmp_root / "subdir/file.pyc", tmp_root, RagConfig.default())

    # relative path upfront
    assert _is_gitignored("is/relative/initially.pyc", tmp_root, RagConfig.default())
...
def test_ignore_if_not_relative_to_workspace_root_dir(tmp_root):
    ...
    assert _is_gitignored("/foo/subdir/file.pyc", tmp_root, RagConfig.default())
    assert _is_gitignored("/foo/subdir/bar.txt", tmp_root, RagConfig.default())
    assert _is_gitignored("/venv/foo.c", tmp_root, RagConfig.default())

def test_literal_entry(tmp_root):
    assert _is_gitignored(tmp_root / "venv/foo.c", tmp_root, RagConfig.default())
    assert not _is_gitignored(tmp_root / "venvfoo.c", tmp_root, RagConfig.default())
