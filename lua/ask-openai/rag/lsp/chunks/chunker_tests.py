...
def _treesitter_chunks_from_file_with_fake_hash(path: Path, options: RAGChunkerOptions) -> list[Chunk]:
...
class TestTreesitterPythonChunker:
    ...
    def test_two_functions_py(self):
        chunks = _treesitter_chunks_from_file_with_fake_hash(test_cases_python / "two_functions.py", RAGChunkerOptions.OnlyTsChunks())
        assert len(chunks) == 2
    ...
    def test_nested_functions_py(self):
        chunks = _treesitter_chunks_from_file_with_fake_hash(test_cases_python / "nested_functions.py", RAGChunkerOptions.OnlyTsChunks())
        assert len(chunks) == 2
    ...
    def test_dataclass_py(self):
        chunks = _treesitter_chunks_from_file_with_fake_hash(test_cases_python / "dataclass.py", RAGChunkerOptions.OnlyTsChunks())
        assert len(chunks) == 1
    ...
    def test_class_with_functions_py(self):
        chunks = _treesitter_chunks_from_file_with_fake_hash(test_cases_python / "class_with_functions.py", RAGChunkerOptions.OnlyTsChunks())
