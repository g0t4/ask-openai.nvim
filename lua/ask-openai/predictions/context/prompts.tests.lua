    ...
    describe("/others", function()
        ...
        describe("extract /k", function()
            ...
            it("middle of prompt", function()
                local includes = prompts.parse_includes("foo /k=5 bar")
                assert.are_equal(5, includes.top_k)
                assert.are_equal("foo bar", includes.rendered_prompt)
            end)

            it("end of prompt", function()
                local includes = prompts.parse_includes("foo bar /k=7")
                assert.are_equal(7, includes.top_k)
                assert.are_equal("foo bar", includes.rendered_prompt)
            end)
        end)
    end)
